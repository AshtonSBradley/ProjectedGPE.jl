...
#= imprint vortex =#
xv = 0.2*Rx/x0; yv = 0.0; σv = 1
testvortex = [xv yv σv]
Δx = (x[2]-x[1])/x0;Δy = (y[2]-y[1])/x0

ix = find(abs(x/x0-xv).<=Δx);ix = ix[1]
iy = find(abs(y/x0-yv).<=Δy);iy = iy[1]
#ξ = x0/(sqrt(μ))
ξ = 1/(sqrt(g*abs2.(ψ)[ix,iy]))
#g*abs2.(ψ)[ix,iy]
makevortex!(ψ,testvort,x/x0,y/x0,ξ)
...
ψ0 = c2x(c0,tinfop);
#pcolormesh(x/x0,y/x0,g*abs2.(ψ0'))
pcolormesh(x/x0,y/x0,angle.(ψ0'))
xlabel("x/x0")
ylabel("y/x0")
colorbar()
