...
function eigmatold(basis,M,x,ω=1.0,α=1.0)
N = length(x)
x = x[:]
...
elseif basis=="Laguerre"
    x=complex(x,0)
  	...
  	wfn_mat[:,1] = laguerre_nm1.*exp(0*sqrt(ω)*x/(2*M)-sqrt(ω)*x/2)
  	wfn_mat[:,2] = laguerre_n.*exp(1*sqrt(ω)*x/(2*M)-sqrt(ω)*x/2)
	...
	if M > 2
  		for nn = 1:M-2
			...
			wfn_mat[:,nn+2]=laguerre_np1.*exp((nn+1)*sqrt(ω)*x/(2*M)-sqrt(ω)*x/2)
  		end
  	end
	wfn_mat = wfn_mat*ω
else
